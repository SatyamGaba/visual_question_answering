...
#         + list(model.fc2.parameters())
    ...
    if args.resume_epoch!=0:  
        load_var = torch.load('./models/model-epoch-{:02d}.pt'.format(args.resume_epoch))
        assert load_var['epoch'] == args.resume_epoch
...
#                 if batch_idx % 100 == 0:
        ...
        if (epoch+1) % args.save_step == 0:
            torch.save({'epoch': epoch+1, 'model': model},
                       os.path.join(args.model_dir, 'model-epoch-{:02d}.pt'.format(epoch+1)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_dir', type=str, default='../COCO-2015/resized_images',
